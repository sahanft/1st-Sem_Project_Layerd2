...
public class CustomerDaoImpl implements CustomerDao {
    ...
    public boolean update(CusromerEntity dto) throws SQLException,ClassNotFoundException {
        return SqlUtil.test("UPDATE customer SET custAddress = ?, custName = ?, custMobile = ? , custPayment =?  WHERE custId = ?",
                dto.getTxtCustAddress(),dto.getTxtCustName(),dto.getTxtCustMobile(),
                dto.getTxtCustPayment(),dto.getTxtCustId());
    }

    @Override
    public boolean delete(CusromerEntity id) throws SQLException, ClassNotFoundException {
        return SqlUtil.test("DELETE FROM customer WHERE custId = ?",id.getTxtCustId());
    }

    @Override
    public ArrayList<CusromerEntity> getAll() throws SQLException, ClassNotFoundException {

        ResultSet rst = SqlUtil.test("SELECT * FROM customer"); // Ensure this method works properly
        ...
        while (rst.next()) {
            ...
            getAllCustomer.add(customerDTO); // Fixed placement of method call
        }
